...
def clip_rawtext(chat_message, need_escape=True):
    """
    处理和裁剪聊天机器人的原始消息文本，以便安全和适当地显示在网页上。

    参数:
        chat_message (str): 机器人发送的原始消息文本。
        need_escape (bool): 指示是否需要转义Markdown特殊字符。默认为True，表示需要转义。

    返回:
        str: 裁剪和处理（可能转义）后的消息文本。
    """

    # 首先，裁剪消息中的<hr>标签以后的内容
    hr_pattern = r'\n\n<hr class="append-display no-in-raw" />(.*?)'
    hr_match = re.search(hr_pattern, chat_message, re.DOTALL)
    # 如果找到<hr>标签，则仅保留该标签之前的内容；否则，保留整个消息
    message_clipped = chat_message[: hr_match.start()] if hr_match else chat_message

    # 其次，处理消息中的代理前缀部分，避免转义
    agent_prefix_pattern = (
        r'(<!-- S O PREFIX -->.*?<!-- E O PREFIX -->)'
    )
    # 使用正则表达式分割消息文本，以分离代理前缀部分和其它文本部分
    agent_parts = re.split(agent_prefix_pattern, message_clipped, flags=re.DOTALL)
    final_message = ""  # 初始化最终消息文本变量
    for i, part in enumerate(agent_parts):
        # 对于非代理前缀的文本部分
        if i % 2 == 0:
            if part != "" and part != "\n":
                # 如果需要转义Markdown特殊字符，则使用escape_markdown函数转义；否则，直接使用原文本
                final_message += (
            ...
            # 处理代理前缀部分，移除其中的 data-fancybox="gallery" 属性
...
def convert_bot_before_marked(chat_message):
    ...
    返回:
        str: 处理后的消息文本，包含适用于显示的HTML标签和Markdown格式。
    """
    # 检查消息是否已经被格式化处理
    if '<div class="md-message">' in chat_message:
        # 如果消息已包含特定的格式化标签，则直接返回，无需进一步处理
        return chat_message
    else:
        ...
        # 使用正则表达式找出所有代码块，```包围的文本视为代码块
        code_block_pattern = re.compile(r"```(.*?)(?:```|$)", re.DOTALL)
        code_blocks = code_block_pattern.findall(chat_message)
        # 分割消息文本为代码块和非代码块部分
        non_code_parts = code_block_pattern.split(chat_message)[::2]

        result = []  # 初始化结果列表，用于存储处理后的文本段
        # 遍历非代码和代码块，按顺序重建消息内容
        for non_code, code in zip(non_code_parts, code_blocks + [""]):
            # 将非空的非代码文本直接添加到结果列表
            if non_code.strip():
                result.append(non_code)
            # 将非空的代码块重新包裹在```中，确保它们正确解析为代码块，然后添加到结果列表
            if code.strip():
                code = f"\n```{code}\n```"
                result.append(code)
        # 将所有处理后的文本段合并，并包裹在<div class="md-message">中，可能用于应用Markdown样式
        result = "".join(result)
        md = f'<div class="md-message">\n\n{result}\n</div>'
        # 返回原始文本HTML表示和Markdown格式化后的消息内容的组合
        return raw + md
...
def escape_markdown(text):
    """
        转义Markdown特殊字符为HTML安全的等价物，以避免在渲染Markdown时产生非预期的格式化效果。
        Escape Markdown special characters to HTML-safe equivalents.

        参数:
            text (str): 需要转义的原始文本字符串。

        返回:
            str: 转义后的字符串，其中Markdown特殊字符被替换为它们的HTML字符实体等价物。
        """
    escape_chars = {
        # ' ': '&nbsp;',
        "_": "&#95;",
